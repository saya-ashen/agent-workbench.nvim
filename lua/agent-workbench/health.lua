local Compat = require("agent-workbench.compat")
...
M.check = function()
    vim.health.start("Agent Workbench")
    ...
    if pcall(require, "img-clip") then
        ...
        vim.health.info(
            "img-clip.nvim not found (`:AgentWorkbenchPasteImage` and clipboard paste interception unavailable)"
        )
    end
    ...
    if tree_cfg.enabled ~= false then
        ...
        if vim.uv.fs_stat(tree_path) then
            vim.health.ok("bundled tree extension found (`:AgentWorkbenchTree`)")
        else
            vim.health.warn("bundled tree extension not found at " .. tree_path, {
                "Reinstall Agent Workbench or set `tree = { enabled = false }` to disable :AgentWorkbenchTree",
            })
        end
    end
    ...
    if compress_cfg.enable ~= false then
        local ImageCompress = require("agent-workbench.image_compress")
