--- Session diff review (:AgentWorkbenchDiff) — review every file the current session
...
local list_win = nil
local list_buf = nil
---@type agent_workbench.DiffReviewSection[] collected sections (list row = index + 1)
local sections = {}
...
---@class agent_workbench.DiffReviewSection
...
---@return agent_workbench.DiffReviewSection[]
function M.parse_sections(output)
    ---@type agent_workbench.DiffReviewSection[]
    local sections = {}
    local current = nil
...
---@param body string[] Section body lines (see agent_workbench.DiffReviewSection.body).
...
---@param section agent_workbench.DiffReviewSection
---@param width integer
---@return string[]
...
---@param sections agent_workbench.DiffReviewSection[]
---@return string[]
...
---@param rendered agent_workbench.DiffReviewSection[]
...
local function first_changed_line(section)
    ...
    local hunk_lines = M.compute_hunk_lines(section.body)
    for i in ipairs(section.body) do
        local new_line = hunk_lines[i]
...
local function fetch_untracked_sections(files, cwd, sections, skipped, idx)
    ...
    if idx > #files then
        if #sections == 0 then
            Notify.warn(":AgentWorkbenchDiff — no diff output for this session's changed files")
            return
        end
    ...
    vim.system({ "git", "rev-parse", "--show-toplevel" }, { cwd = cwd }, function(res)
        vim.schedule(function()
            ...
            if res.code ~= 0 or toplevel == "" then
                Notify.warn(":AgentWorkbenchDiff — current directory is not inside a git work tree")
                return
            end
            local inside, outside = partition_inside(files, toplevel)
            if #inside == 0 then
                Notify.warn(":AgentWorkbenchDiff — no changed files are inside the current git repository")
                return
            end
...
function M.open()
    ...
    if not session then
        Notify.warn(":AgentWorkbenchDiff — no active session")
        return
    end
    local files = vim.tbl_keys(session.changed_files)
    if #files == 0 then
        Notify.warn(":AgentWorkbenchDiff — no files changed in this session yet")
        return
    end
    M.close()
