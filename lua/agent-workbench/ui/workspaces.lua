...
---@class agent_workbench.WorkspaceRow
---@field tab agent_workbench.TabId
---@field index integer
---@field cwd string
---@field name string
...
local BUILTIN_TABLINE = "%!v:lua.require'agent-workbench.ui.workspaces'.tabline()"
...
---@param row agent_workbench.WorkspaceRow
---@return string
...
function M.list()
    local sessions = require("agent-workbench.sessions.manager").list()
    ...
    ---@type agent_workbench.WorkspaceRow[]
    local rows = {}
    for index, tab in ipairs(tabs) do
        ...
        for _, session in ipairs(sessions) do
            if session.workspace_tab == tab or session.workspace_tab == nil and session.cwd == cwd then
                ...
                attention = attention or require("agent-workbench.attention").count_for_session(session) > 0
            end
        end
...
function M.create()
    vim.ui.input({
        ...
        if not path or vim.fn.isdirectory(path) ~= 1 then
            require("agent-workbench.notify").error("Workspace path is not a directory: " .. input)
            return
        end
        vim.cmd.tabnew()
