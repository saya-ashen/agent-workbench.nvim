--- Sessions overview (:AgentWorkbenchSessions) — a live list of all active pi sessions.
---
...
---@alias agent_workbench.SessionsListStatus "busy"|"compacting"|"idle"|"exited"

---@class agent_workbench.SessionsListRow
---@field session_id integer
---@field tab agent_workbench.TabId Last active tab, kept for compatibility in status events.
---@field status agent_workbench.SessionsListStatus
...
---@type table<agent_workbench.TabId, integer> list window per tab
local wins = {}

---@type agent_workbench.SessionsListRow[] rows of the last render (index == buffer line)
local rows = {}
...
---@type table<agent_workbench.Session, { name: string|false, first_message: string|false, pending: boolean? }|string>
...
---@param row agent_workbench.SessionsListRow
---@param tick integer
---@return string
...
---@param attention_count fun(tab: agent_workbench.TabId, session?: agent_workbench.Session): integer
---@param name_of fun(session: agent_workbench.Session): string?
---@param flags_of? fun(session: agent_workbench.Session): { done: boolean, error: boolean }
---@return agent_workbench.SessionsListRow[]
function M.build_rows(sessions, attention_count, name_of, flags_of)
    ---@type agent_workbench.SessionsListRow[]
    local out = {}
...
---@type fun(session: agent_workbench.Session)
local fetch_name
...
---@type table<agent_workbench.Session, { done: boolean, error: boolean }>
...
function M.on_agent_end(session)
    local f = session_flags(session)
    if not f.error and not require("agent-workbench.sessions.manager").is_current(session) then
        f.done = true
    end
...
local function rename_under_cursor()
    ...
    require("agent-workbench.ui.dialog").input({ title = "Session Name", default = current }, function(value)
