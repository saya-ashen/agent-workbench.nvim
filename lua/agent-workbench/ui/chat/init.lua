...
---@field _agent agent_workbench.ChatAgent
---@field _layout agent_workbench.ChatLayout
...
---@field _zen agent_workbench.Zen
...
local Zen = require("agent-workbench.ui.chat.zen")
local Worksheet = require("agent-workbench.ui.chat.terminal.worksheet")
local PromptHistory = require("agent-workbench.prompt_history")

---@class agent_workbench.CompactionQueuedMessage
---@field text string
...
function Chat:_set_keymaps()
    ...
    local function capture_history_view()
        ...
        if hbuf == self:history_buf() and vim.api.nvim_win_is_valid(win) and vim.api.nvim_win_get_buf(win) == hbuf then
    ...
    -- the agent (same as :AgentWorkbenchAbort). Insert mode always leaves insert (returns
...
---@class agent_workbench.ChatShowOpts
---@field loading? boolean Show "Loading session…" placeholder

---@param opts? agent_workbench.ChatShowOpts
...
---@param other agent_workbench.Chat
...
function Chat:_auto_dispatch_attention_on_prompt_focus()
    ...
    return require("agent-workbench.attention").open_next_for_tab(self._tab)
end
...
--- (same as :AgentWorkbenchAbort / pi.abort()). A timer disarms automatically once the
...
function Chat:_handle_abort_esc()
    if not (self._streaming or self._retrying) then
        ...
        if self._bash_running then
            require("agent-workbench").abort_bash()
            return
        end
        self:_disarm_abort_esc()
        return
    ...
    if self._abort_esc_at then
        ...
        vim.schedule(function()
            if abort_retry then
                require("agent-workbench").abort_retry()
            else
                require("agent-workbench").abort()
            end
        end)
        return
    end
...
---@param entry agent_workbench.CompactionQueuedMessage
...
---@return agent_workbench.PromptHistoryStore?
...
function Chat:_send_message(queue_type)
    ...
    if require("agent-workbench.slash_commands").execute(text) then
        self._prompt:clear_text()
        return
    end
...
---@param req_id string
---@param res agent_workbench.RpcEvent
...
---@param entries agent_workbench.PendingQueueEntry[]
---@param text string
...
function Chat:refresh_prompt_attention()
    self._layout:refresh_prompt_attention(
        self._prompt:is_request_mode() or require("agent-workbench.attention").has_attention(self._tab)
    )
end
