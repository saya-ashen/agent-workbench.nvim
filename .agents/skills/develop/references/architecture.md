...
Every UI mutation that originates from an RPC callback **must** be on a `vim.schedule` (the callback is not on the main loop). The History buffer remains the transcript projection, while message-level Markdown blocks retain only the source/anchor/decoration metadata needed to recompile one isolated block.
...
- **History is not one Markdown document.** `pi-chat-history` has no global Markdown parser. Every user message and assistant text segment is compiled in an isolated scratch buffer through `markview.parser`; tools/thinking/status never enter that parser. Never reintroduce whole-History parsing or synthetic tool fences.
- **Conceal is scoped by block Extmarks.** History windows use `conceallevel=2`, but only Markdown-block decorations conceal source markers. Tool text has no Markdown decorations and therefore stays verbatim even when it contains fences, headings, or tables. Cursor reveal toggles only decorations owned by the active semantic range; it must reuse the stored plan and never parse on `CursorMoved`.
- **Extmarks carry the structure.** Tool borders, highlights, spinners, thinking anchors, and collapse ranges use namespace `pi-chat`; message Markdown uses `agent-workbench/markdown` and tracks plan-indexed decoration IDs per block; assistant text rails use `agent-workbench/assistant-blocks` so they remain independent of parser availability. When replacing lines, preserve structural marks and let block anchors follow insertions; never clear an entire Markdown namespace to refresh one message.
...
- **Buffer-owned sessions.** Each listed History buffer owns one `{ rpc, chat }`; `require("agent-workbench.sessions.manager").get()` resolves current component buffer or active tab view. Cleanup happens on History buffer deletion and `VimLeavePre`; tab close only detaches view.
...
- **Config is read live.** Always `require("agent-workbench.config").options` at call time. Caching a config value at module load (G20) yields stale values after `setup()`.
- **Lazy loading.** `pi` is loaded on first use (a keymap/command). In tests, `wait_for` the chat buffers rather than assuming `require("agent-workbench")` already ran; or force it with `pcall(require,"pi")` / `pi.show()`.
