...
- **Conceal is scoped by block Extmarks.** History windows use `conceallevel=2`, but only Markdown-block decorations conceal source markers. Tool text has no Markdown decorations and therefore stays verbatim even when it contains fences, headings, or tables.
- **Extmarks carry the structure.** Tool borders, highlights, spinners, thinking anchors, and collapse ranges use namespace `pi-chat`; message Markdown uses `agent-workbench/markdown` and tracks decoration IDs per block. When replacing lines, preserve structural marks and let block anchors follow insertions; never clear an entire Markdown namespace to refresh one message.
