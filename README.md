...
- **Sessions overview (`:PiSessions`).** A live list of all active sessions — one row per tab: a single status dot at the left edge (colored and animated per state) with the session's display name right after it. One shared buffer across all tabs: each tab opens its own window on it, so every view updates together as sessions start, stream, finish, or ask for attention. `<CR>` jumps to a session's tab and opens its chat; `r` renames the session under the cursor; `R` re-fetches names; `?` toggles a shortcut help overlay; `q` closes. Window placement follows the tab's chat layout (side vs float) and is configurable via `sessions_list`.
...
- A resumed session (`:PiResumeSession` / `:PiContinueSession`) adopts the model stored in its session file, as resolved by the backend.
