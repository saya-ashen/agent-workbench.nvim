...
Keys inside the list: `<CR>` / `o` jump to that session's tab and open its chat, `r` re-fetches session names, `?` toggles a shortcut help overlay, `q` closes the window. The list is read-only.
