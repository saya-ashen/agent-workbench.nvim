...
- a single **status dot** at the left edge, colored and animated per state: blinking in the agent color while the agent works, slow-blinking in the compaction color while compacting, steady warning color when the session needs your attention, steady dim when idle, steady error color if the process died,
