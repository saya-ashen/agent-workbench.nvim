...
- **Session tree navigation (`:PiTree`).** Jump back to any past point in the current session — a picker lists the conversation entries (depth-indented, current point marked `●`, branch labels shown), you choose whether to summarize the abandoned branch, and the chat rebuilds from the new branch; picking a user message puts its text back in the prompt for editing and resending. This is the π equivalent of the TUI's `/tree`, bridged into RPC mode through a bundled pi extension (`extensions/tree.ts`), so no extra setup is needed. A bare `/tree` typed in the prompt opens it too. Config: `tree.enabled`.
