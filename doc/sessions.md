...
Keys inside the list: `<CR>` / `o` jump to that session's tab and open its chat using the first-entry or saved-History focus behavior, `r` renames the session under the cursor (same as `:PiSessionName`, without leaving the list), `R` re-fetches session names, `?` toggles a shortcut help overlay, `q` closes the window.
