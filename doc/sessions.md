...
`:AgentWorkbenchWorkspaceSidebar` toggles a collapsible workspace explorer (right side, 38 columns by default). Its compact tree-style rows show a shortened `~/...` cwd, then list that workspace's open session History buffers, ordinary listed buffers, and—when the built-in Pi backend is active—persisted Pi session files. Backend-owned history from external backends stays available through `:AgentWorkbenchContinue` and `:AgentWorkbenchResume`; the explorer never mixes unrelated Pi JSONL files into an external backend's workspace. Open-session rows show a backend title (falling back to `session <id>`), a state-specific icon, and a right-aligned `running`, `compacting`, `attention`, `idle`, or `stopped` state. Persisted Pi sessions that are not already open appear under `Today`, `Yesterday`, `Last 7 days`, and `Older`; `Today` starts expanded while the other time groups start collapsed. Historical rows use the persisted name or first user message and show a right-aligned time, weekday, or date. Their metadata is loaded lazily when the workspace expands and cached across ordinary redraws; `R` forces a reload without making routine updates rescan session files. Opening a historical row with `<CR>` / `l` switches to its workspace and resumes it as a separate live session, or reuses its History buffer if it is already open; existing sessions are never replaced. Ordinary buffers show their basename, filetype icon and color from `nvim-web-devicons` when available, and a `modified` marker when changed; without devicons they use a generic file icon. On workspace rows, both `h` and `l` toggle expansion. On time-group rows, `l`, `<CR>`, `e`, and `<Tab>` toggle expansion; `h` collapses an open group or moves back to the workspace. On open-session rows, `h` collapses the parent workspace, `l` activates the session, and `p` opens or closes a live, read-only History preview. Preview keeps focus in the sidebar, does not activate the session, switch tabs, or change cwd, and therefore does not trigger a workspace's `direnv` rebuild; its title shows current session state and its content follows background output. Activation focuses the prompt with History scrolled to latest output on first entry or restoring History at its last moved cursor afterward. On buffer rows, `<CR>` / `l` switches to the buffer. `<CR>` switches a workspace or opens its selected item. `d` acts on open resources only: a workspace row confirms before closing its tab and stopping all its sessions, an open-session row confirms before stopping and deleting its History buffer, and a buffer row deletes directly; historical files are not deleted from the explorer. Closing the last workspace is refused; ordinary buffers from a closed workspace remain available. `a` creates a session in the selected workspace, `A` opens the new-workspace path input, `o` switches and closes the sidebar, `R` reloads workspaces, historical metadata, buffers, and open-session titles, `?` toggles key help, and `q` closes. The sidebar is a native scratch buffer and split; it does not require Snacks or Neo-tree. Configure `workspace_sidebar.position` (`"left"` or `"right"`) and `workspace_sidebar.width`.

Buffers are tracked by workspace ownership. On tab switches, Agent Workbench temporarily lists only buffers owned by the current workspace, keeping bufferline, `:bnext`, and `:bprevious` scoped to that workspace without deleting hidden buffers. The workspace sidebar still groups tracked buffers from every workspace and keeps session History buffers with their creating workspace. A normal buffer can belong to multiple workspaces when entered there. Entering a session History buffer directly restores its last moved cursor and expanded/collapsed block folds, or opens at latest output when no cursor was recorded. `:AgentWorkbenchMoveBuffer {tab-number}` moves the current ordinary buffer to another workspace; π History buffers cannot be moved. Closing a tab outside the sidebar only detaches its workspace view and keeps sessions alive. Confirming `d` on a sidebar workspace row additionally stops that workspace's sessions; either path preserves ordinary buffers.
...
The URI starts as `agent://<project>/new-<id>/transcript` for a new session and changes to persisted session ID once RPC state provides its session file. Each URI owns its transcript buffer and History renderer state. A listed History buffer starts as `π session <live-id>`, then becomes `π <session-name> [<live-id>]` when the backend name is known, falling back to the first user message. Titles are single-line and bounded for bufferline and file-tree readability; a Neovim name collision adds the buffer ID. Its `agent://...` URI remains in the buffer-local `pi_session_uri` field and workspace registry. `:edit agent://.../transcript` reuses an existing resource and activates its session in the owning workspace, or attaches it to the current workspace when the owner was closed. History remains separate from prompt and attachment buffers. In buffer layout, opening a normal file replaces the current view and hides chat layout without stopping session; switching back to its named History buffer restores History and prompt.
...
| `:AgentWorkbenchSessionName [name]` | `pi.set_session_name(name?)` | Set a human-readable display name for the current session. Without an argument, opens an input dialog prefilled with the current name. Names appear in the History buffer title, tabline integrations, and `:AgentWorkbenchResume` picker. |
