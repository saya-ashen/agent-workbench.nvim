...
When you run several sessions, `:PiSessions` gives you a single dashboard of everything live. It lists active session buffers, including multiple sessions in one tab. Selecting a row activates that session in current tab with History focused in Normal mode.
...
Keys inside the list: `<CR>` / `o` jump to that session's tab and open its chat on History in Normal mode, `r` renames the session under the cursor (same as `:PiSessionName`, without leaving the list), `R` re-fetches session names, `?` toggles a shortcut help overlay, `q` closes the window.
