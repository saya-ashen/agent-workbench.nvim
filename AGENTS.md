# Agent Workbench — Agent Guidelines

Neovim plugin (Lua) — an independently maintained workspace frontend for coding agents, currently powered by the [pi coding agent](https://github.com/earendil-works/pi). Provides workspace-aware chat, diff review, session management, and persistent shell workflows inside Neovim.
...
Each tool has a renderer in `tools.lua` (`agent_workbench.ToolRenderer`). Renderers define `on_start`/`on_end` for content, plus `input_visible`/`output_visible` thresholds for auto-collapse. Inline tools (like `read`) render as a single line. Custom renderers can be added to the `renderers` table.
...
Each listed History buffer owns one session with its own `Rpc` and `Chat`; a tab can hold multiple sessions. The active view resolves from the current component buffer or workspace tab. Cleanup happens on History deletion and `VimLeavePre`; tab close detaches the workspace view.
...
- **Config access**: always go through `require("agent-workbench.config").options`, never cache config values at module load time.
