    ...
    describe("command construction", function()
        ...
        it("magick: png output omits -quality", function()
            assert.are_same(
                ...
                Compress._build_cmd(
                    ...
                    "in.png",
                    "out.png",
                    { max_dimension = 1568, quality = 80, format = "png" }
                )
            )
        end)
        ...
        it("ffmpeg: aspect-preserving scale and mapped qscale", function()
            assert.are_same({
                ...
                "in.png",
                "-vf",
                "scale=w=1568:h=1568:force_original_aspect_ratio=decrease",
                "-q:v",
                "8", -- quality 80 → 2 + 29*0.2
                "out.jpg",
            }, Compress._build_cmd("ffmpeg", "in.png", "out.jpg", opts))
        end)

        it("ffmpeg: webp uses -quality", function()
            local cmd = Compress._build_cmd(
                "ffmpeg",
                "in.png",
                "out.webp",
                { max_dimension = 0, quality = 80, format = "webp" }
            )
