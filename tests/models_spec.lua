...
describe("models.resolve_entries", function()
    ...
    it("matches a string entry by exact ID", function()
        local resolved = require("agent-workbench.models").resolve_entries({ "k3" }, all_models)
    ...
    it("matches a string entry by canonical provider/modelId", function()
        local resolved =
            require("agent-workbench.models").resolve_entries({ "opencode-go/deepseek-v4-flash" }, all_models)
        assert.are.same(1, #resolved)
    ...
    it("matches every provider copy of a bare ambiguous ID", function()
        local resolved = require("agent-workbench.models").resolve_entries({ "deepseek-v4-flash" }, all_models)
    ...
    it("dedupes across providers only within the same entry form", function()
        ...
        local resolved = require("agent-workbench.models").resolve_entries(
    ...
    it("warns when a string entry matches nothing", function()
        local resolved = require("agent-workbench.models").resolve_entries({ "no-such-model" }, all_models)
