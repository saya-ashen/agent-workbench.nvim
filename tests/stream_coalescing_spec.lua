...
describe("stream coalescing", function()
  ...
  it("keeps text and tool blocks in stream order", function()
    ...
    h:on_agent_start(nil)
    pump(30)
    -- Line-oriented deltas (as real agent text arrives): each chunk ends in
    -- a newline so text and tool blocks occupy distinct rows.
    ...
    assert.is_true(tool[1] < mid[1], "tool block before mid text")
  ...
  it("agent_end table scan sees the fully drained text", function()
    ...
    pump(80)
    local buf = h:buf()
    assert.is_true(text_of(buf):find("─", 1, true) ~= nil or #rows_with(buf, "│") > 0,
